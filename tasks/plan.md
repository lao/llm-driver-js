...
package.json (name `llmwrapper`, version 0.1.0, MIT, `engines.node >= 20`,
...
`src/types.ts`, `src/errors.ts` (LLMWrapperError per SPEC), `src/config.ts`
...
**Accept:** vitest green; all four pairs construct; every invalid case yields LLMWrapperError with correct `code`.
...
Normalize SDK errors → LLMWrapperError (`api_error` with status+providerCode;
